		...
		<div class="sh-bl archive <?php echo $class; ?>">
			...
			<?php
				if( $ar['type_deal'] == 'buy' || $ar['type_deal'] == 'rent' ){
					echo '<a href="' . $slug . '" class="btn">';
					_e( 'Show All Properties', THEME_NAME );
					echo '</a>';
				}
			?>
			</div>
...
function wpcf7_form_replace($form){
	global $post;
	if( function_exists('get_field' ) ){
		$field = get_field('Property_Ref_No', $post->ID );
		return str_replace('{unit}', $field, $form);
	}
}
